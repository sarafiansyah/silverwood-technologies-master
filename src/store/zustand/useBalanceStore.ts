...
export const useBalanceStore = create(
    persist<BalanceState>(
        (set, get) => ({
            ...
            removeLimit: (title) =>
                set((state) => {
                    const limitToRemove = state.limits.find(
                        (l) => l.title === title,
                    );
                    ...
                    return {
                        ...
                        currentBalance:
                            state.currentBalance + limitToRemove.value,
                    };
                }),

            editLimit: (oldTitle, newTitle, newValue) =>
                set((state) => {
                    const target = state.limits.find(
                        (l) => l.title === oldTitle,
                    );
            ...
            setHeirloomsTotal: (heirloomsTotal) =>
                set((state) => {
                    const otherLimits = state.limits.filter(
                        (l) => l.title !== "Heirlooms",
                    );
                    const updatedLimits = [
                        ...otherLimits,
                        { title: "Heirlooms", value: heirloomsTotal },
                    ];
                    ...
                    return {
                        ...
                        currentBalance:
                            state.totalIncome -
                            updatedLimits.reduce((a, b) => a + b.value, 0),
                    };
                }),
            ...
            setTotalIncome: (newTotal) =>
                set((state) => ({
                    ...
                    currentBalance:
                        newTotal -
                        state.limits.reduce((sum, l) => sum + l.value, 0),
                })),
